...
def test_wellformedness_verify_request_good():
    ...
    assert isinstance(verify_wellformedness(string1).json(),dict)

def test_wellformedness_verify_request_bad():
    assert verify_wellformedness_request(string2).status_code == 200
    assert isinstance(verify_wellformedness(string2).json(),dict)

def test_wellformedness_verify_request_empty():
    assert verify_wellformedness_request(string3).status_code == 200
    assert isinstance(verify_wellformedness(string3).json(),dict)

# Whitebox testing
def test_wellformedness_invoice_correct():
    assert verify_wellformedness(string1).status_code == 200
    assert verify_wellformedness(string1).json() == {'broken_rules': 'No broken rules found. Invoice is well-formed!'}

def test_wellformedness_invoice_wrong():
    assert verify_wellformedness(string2).status_code == 200
    assert verify_wellformedness(string2).json() =={'broken_rules': ["ERROR ON LINE 23: Failed to parse QName 'cbc:'", 
...
def test_wellformedness_invoive_empty():
    assert verify_wellformedness(string3).status_code == 200
    assert verify_wellformedness(string2).json() == {"broken_rules": "The provided file is empty"}